...
# Clone and build
git clone https://github.com/pingzi/ida-mcp-skill.git
cd ida-mcp-skill
cargo build --release
```
...
The binary is at:
- Linux/macOS: `target/release/ida-mcp-skill`
- Windows: `target/release/ida-mcp-skill.exe`
...
./target/release/ida-mcp-skill

# Streamable HTTP (multi-client, SSE)
./target/release/ida-mcp-skill serve-http --bind 127.0.0.1:8765

# CLI probe (test IDA connection)
./target/release/ida-mcp-skill probe --path /path/to/binary --list 10
```
