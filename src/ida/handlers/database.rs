...
pub fn handle_open(
    ...
    // Always clean up stale lock files from dead processes (pid no longer running).
    // Previously this required force=true; now it's automatic since dead-pid locks
    // are safe to remove without user confirmation.
    if let Some(stale) = clean_stale_mcp_lock(&expanded) {
        info!(
            path = %stale.path.display(),
            pid = stale.pid,
            reason = %stale.reason,
            "Cleaned up stale lock file from dead process"
        );
    }
