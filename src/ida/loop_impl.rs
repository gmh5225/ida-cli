...
/// Default deadline for requests (same as DEFAULT_TIMEOUT_SECS in worker.rs).
const DEFAULT_DEADLINE_SECS: u64 = 120;

pub fn run_ida_loop(rx: mpsc::Receiver<EnqueuedRequest>) {
    ...
    while let Ok(enqueued) = rx.recv() {
        let wait_ms = enqueued.enqueued_at.elapsed().as_millis();
        let req = enqueued.request;

        // Fix 2: Skip requests whose caller already disconnected (oneshot cancelled).
        ...
        // Fix 3: Skip requests that have been queued longer than the deadline.
        // The caller has almost certainly timed out by now.
        if !req.is_control() && wait_ms > (DEFAULT_DEADLINE_SECS as u128 * 1000) {
            warn!(
                request = req.type_name(),
                wait_ms,
                deadline_secs = DEFAULT_DEADLINE_SECS,
                "Skipping expired request (exceeded deadline)"
            );
            continue;
        }
